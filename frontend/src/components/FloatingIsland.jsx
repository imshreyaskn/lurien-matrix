...
export default function FloatingIsland() {
  ...
  return (
    <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex items-center p-2 bg-luma-100/50 backdrop-blur-2xl border border-white/10 rounded-full shadow-[0_20px_40px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.1)] transition-transform duration-500 hover:scale-[1.02]">
      {/* Brand / Status */}
      <div className="flex items-center gap-3 pl-4 pr-6 border-r border-white/10">
        ...
        <div className={`w-2 h-2 rounded-full shadow-[0_0_10px_currentColor] ${error ? 'bg-status-offline text-status-offline' : 'bg-status-online text-status-online'}`} title={error ? 'System Offline' : 'System Online'} />
      </div>
      ...
      <div className="flex items-center px-4 gap-2">
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `relative group flex items-center justify-center w-12 h-12 rounded-full transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] ${
                isActive
      ...
      <div className="flex items-center pl-6 pr-2 border-l border-white/10">
        <button
          onClick={logout}
          className="flex items-center justify-center w-10 h-10 rounded-full text-luma-500 hover:text-firewall-red hover:bg-firewall-red/10 transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)]"
