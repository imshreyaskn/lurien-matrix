...
Get-ChildItem -Path $hf_dir | Where-Object { $_.Name -notin @('.git', 'README.md', '.gitattributes', 'models') } | Remove-Item -Recurse -Force

# Copy files from backend to HF repo excluding models and venv
Get-ChildItem -Path $backend_dir | Where-Object { $_.Name -notin @('models', 'venv', '__pycache__') } | Copy-Item -Destination $hf_dir -Recurse -Force
